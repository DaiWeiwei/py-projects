...
class DataAnalysisService(object):
    ...
    def combo_login(self, username='10022510', password='zte', uri=REST_USER_LOGIN):
    ...
    def get_report(self, local_path, abs_url):
        ...
        if not os.path.isfile(local_fpath):
            local_fpath = os.path.abspath(local_fpath) +'\\upgrade_report.html'

        with open(local_fpath, 'a') as f:
            f.write(res.content)
        logger.info(u'########################################################################')
...
if __name__ == '__main__':
    ...
    rest.combo_login()
    rest.compare_files_upload(base_path=r'E:\umac_local\mme_19\before_upgrade\html\before_config_data.html',
                              new_path=r'E:\umac_local\mme_19\after_upgrade\html\after_config_data.html')
    rest.select_rule(0, r'E:\umac_local\umac_html_rule.xlsx')
    report_url = rest.compare()
    local_html = rest.get_report(r"E:\umac_local", report_url)
    print local_html
